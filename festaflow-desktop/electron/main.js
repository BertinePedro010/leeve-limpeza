...
function startApi() {
  ...
  const env = {
    ...
    JWT_SECRET: process.env.JWT_SECRET || "festaflow-desktop-secret",
  };

  const entry = backendEntry();
  if (fs.existsSync(entry)) {
    apiProcess = fork(entry, [], { env, stdio: "pipe" });
  }
}
...
function createWindow() {
  ...
  if (isDev) win.loadURL(frontendIndex());
  else win.loadFile(frontendIndex());
}
