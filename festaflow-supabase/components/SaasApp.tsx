...
type Tab = "dashboard" | "clients" | "employees" | "services" | "orders" | "calendar" | "finance" | "reports" | "branches";
...
type Me = { id: string; name: string; email: string; role: string; isGlobalAdmin: boolean };
...
function OrdersView({ orders, clients, employees, services, branchId, reload }: { orders: Order[]; clients: Client[]; employees: Employee[]; services: Service[]; branchId: string | null; reload: () => Promise<void> }) {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap justify-between gap-2"><p className="text-sm text-slate-500">OS com multiplos atendimentos, recorrencia, equipe, assinatura e impressao/PDF.</p><div className="flex gap-2"><button onClick={() => setRecurrenceOpen(true)} className="rounded-xl border border-indigo-300 px-4 py-2 font-black text-indigo-600">Nova Recorrencia</button><button onClick={() => { setEditing(null); setOpen(true); }} className="rounded-xl bg-indigo-600 px-4 py-2 font-black text-white">Nova OS</button></div></div><div className="overflow-hidden rounded-2xl border bg-white"><table className="w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="p-4">Codigo</th><th>Cliente</th><th>Data</th><th>Atendimentos</th><th>Pagamento</th><th>Status</th><th className="text-right">Valor</th><th className="p-4 text-right">Acoes</th></tr></thead><tbody>{orders.map((o) => <tr key={o.id} className="border-t"><td className="p-4 font-mono font-black">{o.code}</td><td>{o.client?.name}</td><td>{new Date(o.eventDate).toLocaleDateString("pt-BR")}</td><td>{o.appointments?.length ?? 0}</td><td className="text-xs">{paymentMethodLabel(o)}</td><td><Badge status={o.status} /></td><td className="text-right font-black text-indigo-600">{money(o.totalAmount)}</td><td className="p-4 text-right"><button onClick={() => setPrint(o)} className="mr-2 font-bold">PDF</button><button onClick={() => { setEditing(o); setOpen(true); }} className="mr-2 font-bold text-indigo-600">Editar</button><button onClick={() => removeOrder(o.id)} className="font-bold text-rose-600">Excluir</button></td></tr>)}</tbody></table></div>{open && <OrderFormModal order={editing} orders={orders} clients={clients} employees={employees} services={services} branchId={branchId} onClose={() => setOpen(false)} reload={reload} />}{print && <PrintOrder order={print} close={() => setPrint(null)} />}{recurrenceOpen && <RecurrenceModal clients={clients} services={services} employees={employees} branchId={branchId} close={() => setRecurrenceOpen(false)} onCreated={reload} />}</div>;
}
...
function CalendarView({ branchId, employees, clients, services, orders, reload }: { branchId: string | null; employees: Employee[]; clients: Client[]; services: Service[]; orders: Order[]; reload: () => Promise<void> }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <div className="overflow-hidden rounded-2xl border bg-white lg:col-span-2"><div className="grid grid-cols-7 bg-slate-50 text-center text-xs font-black uppercase text-slate-500">{["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sab"].map((x) => <div key={x} className="py-3">{x}</div>)}</div><div className="grid grid-cols-7 auto-rows-[110px]">{days.map((d, i) => <button type="button" key={i} disabled={!d} onClick={() => d && setSelectedDay(d)} className={`border-t border-r p-2 text-left ${d === selectedDay ? "bg-indigo-50" : ""}`}>{d && <><b className="text-xs">{d}</b><div className="mt-2 space-y-1">{byDay(d).slice(0, 3).map((a) => <div key={a.id} className="truncate rounded-lg bg-indigo-100 p-1 text-[10px] font-bold text-indigo-700">{a.startTime} {a.order.client?.name}</div>)}{byDay(d).length > 3 && <p className="text-[10px] font-bold text-slate-400">+{byDay(d).length - 3}</p>}</div></>}</button>)}</div></div>
      <div className="rounded-2xl border bg-white p-5"><h4 className="font-black capitalize">Atendimentos - {selectedLabel}</h4>{selectedAppointments.length === 0 && <p className="mt-4 text-sm text-slate-500">Nenhum atendimento neste dia.</p>}<div className="mt-4 space-y-3">{selectedAppointments.map((a) => <button type="button" key={a.id} onClick={() => openAppointment(a)} className="w-full rounded-xl border p-3 text-left hover:border-indigo-300 hover:bg-indigo-50/40"><div className="flex items-center justify-between"><b className="text-sm">{a.startTime} - {a.endTime}</b><Badge status={a.status} /></div><p className="mt-1 text-sm font-bold text-slate-800">{a.order.client?.name}</p><p className="text-xs text-slate-500">{a.order.items[0]?.service?.name} - OS {a.order.code}</p><p className="text-xs text-slate-500">{a.employee?.name || "Equipe da OS"} - {a.branch?.name}</p></button>)}</div></div>
    </div>
