...
const nav: Array<[Tab, string]> = [["dashboard", "Dashboard"], ["orders", "Ordens de Servico"], ["calendar", "Calendario"], ["clients", "Clientes"], ["employees", "Funcionarios"], ["services", "Servicos"], ["finance", "Financeiro"], ["reports", "Relatorios"], ["training", "Treinamento"]];
...
function canUseModule(me: Me | null, id: Tab): boolean {
  // "training" is help content, not business data - every authenticated user
  // gets it regardless of module permissions (see components/TrainingView.tsx).
  if (id === "dashboard" || id === "training" || id === "branches" || id === "users") return true;
  if (!me) return true;
...
function validateClientForm(form: ClientForm): Record<string, string> {
  ...
  // Length thresholds mirror clientSchema exactly (lib/validators.ts) - a
  // 1-character value used to pass the native HTML `required` attribute
  // (non-empty) but still fail the server's `.min(2)`, landing the user back
  // on a round-trip 422 instead of an immediate, specific client-side error.
  if (form.name.trim().length < 2) errors.name = "Informe o nome do cliente (minimo 2 caracteres).";
  if (form.email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) errors.email = "O e-mail informado nao e valido. Exemplo: cliente@empresa.com.";
  if (form.addressStreet.trim().length < 2) errors.addressStreet = "Informe a rua/logradouro do endereco (minimo 2 caracteres).";
  if (!form.addressNumber.trim()) errors.addressNumber = "Informe o numero do endereco.";
  if (form.addressNeighborhood.trim().length < 2) errors.addressNeighborhood = "Informe o bairro do endereco (minimo 2 caracteres).";
  if (form.addressCity.trim().length < 2) errors.addressCity = "Informe a cidade do endereco (minimo 2 caracteres).";
...
function ClientsView({ data, branchId, reload, loading, openClientId, onOpenClientHandled }: { data: Client[]; branchId: string | null; reload: () => Promise<void>; loading: boolean; openClientId?: string | null; onOpenClientHandled?: () => void }) {
  ...
  // Hits GET /api/clients?search=... (branch-scoped server-side, see
  // app/api/clients) instead of filtering the already-loaded `data` list in
  // the browser - re-run after a save/delete too, so an edited client's row
  // (name/document) stays consistent with what's currently on screen.
  async function runSearch(term: string) {
    if (!term || !branchId) { setSearchResults(null); return; }
    setSearching(true);
    try {
      const results = await api<Client[]>(`/api/clients?branchId=${branchId}&search=${encodeURIComponent(term)}`);
      setSearchResults(results);
      setSearchError("");
    } catch (err) {
      setSearchError(err instanceof Error ? err.message : "Nao foi possivel pesquisar agora. Tente novamente.");
    } finally {
      setSearching(false);
    }
  }
  useEffect(() => {
    ...
    setSearchResults(null); // clear stale/previous-branch results immediately, new ones (if any) land when the debounced fetch resolves
    if (!term) { setSearchError(""); return; }
    const timer = setTimeout(() => runSearch(term), 300);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [search, branchId]);
  const displayedClients = isSearching ? (searchResults ?? []) : data;
  // Opened here from an OS form's "Editar cliente" action - pop the editor for
  // that client as soon as its row is available in the loaded list. Waits
  // for `loading` to clear before deciding "not found": right after a branch
  // switch `data` is briefly `[]` while the new branch's clients are still
  // being fetched, and resolving "not found" against that empty array would
  // silently drop the request instead of waiting for the real list to land.
  // Once loading has finished and the client still isn't in `data`, it
  // genuinely isn't visible from the active branch (wrong branch, deleted,
  // or a stale id) - surfaced as a specific error instead of doing nothing.
  useEffect(() => {
    if (!openClientId || loading) return;
    const target = data.find((c) => c.id === openClientId);
    if (target) {
      setEditing(target); setError(""); setFieldErrors({}); setForm(clientToForm(target)); setOpen(true);
    } else {
      setError("Nao foi possivel carregar este cliente porque ele nao pertence a filial ativa ou o registro foi removido. Troque para a filial correta ou verifique se o cliente ainda existe.");
    }
    onOpenClientHandled?.();
  }, [openClientId, data, loading, onOpenClientHandled]);
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (isSearching) await runSearch(search.trim());
    } catch (err) {
...
function RecurrenceModal({ clients, services, employees, branchId, close, onOrderCreated, reload, onEditClient }: { clients: Client[]; services: Service[]; employees: Employee[]; branchId: string | null; close: () => void; onOrderCreated: (order: Order) => Promise<void>; reload: () => Promise<void>; onEditClient?: (clientId: string) => void }) {
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    if (saving) return; // ignore a second click/Enter while the first submit is still in flight
...
function PrintOrder({ order, close }: { order: Order; close: () => void }) {
  ...
  // order.total is the API's own already-computed real total (see
  // lib/order-total.ts) - reused here instead of recomputing the same
  // nonCancelledCount x totalAmount formula a second time client-side.
  const recurrenceTotal = Number(order.total);
  return <><Modal title={`OS ${order.code}`} onClose={close}><div className="print-page mx-auto max-w-4xl rounded-2xl border p-8">
    ...
    {schedule && <section className="mt-6 break-inside-avoid">
      ...
      <div className="mt-2 grid gap-1 text-sm md:grid-cols-2">
        ...
        {/* Distinct from "Servicos contratados" above (preco real do servico)
            and from "valor por atendimento" below (regra existente,
            inalterada: order.totalAmount x ocorrencias) - este e o valor
            mensal contratado para a recorrencia (RecurringSchedule.price). */}
        <p className="md:col-span-2 font-bold"><b>Valor mensal da recorrencia:</b> {money(schedule.price)}</p>
