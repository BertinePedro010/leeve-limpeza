...
import { statusLabels, money, dateOnly, dateOnlyLabel, api, ApiError, Badge, Modal, Stat, CrudShell, ActionButtons, Input, NumberInput, Text, Select, Save, paymentMethodLabel, paymentMethodOptions, EmployeeMultiSelect, CepField, brazilStateOptions, MultiDatePicker } from "@/components/ui";
...
function SaasAppShell() {
  ...
  return <div className="flex h-screen overflow-hidden bg-slate-50 text-slate-800">
    ...
    <main className="flex-1 overflow-y-auto"><header className="no-print sticky top-0 z-20 border-b border-slate-200 bg-white/90 px-4 py-4 backdrop-blur lg:px-8"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><button onClick={() => setSidebarOpen(true)} aria-label="Abrir menu" className="rounded-xl border border-slate-200 p-2 text-slate-600 lg:hidden">☰</button><div><h2 className="text-xl font-black">{visibleNav.find(([id]) => id === tab)?.[1]}</h2><p className="text-xs font-bold uppercase tracking-wide text-indigo-600">{activeBranch?.name || "Selecione uma filial"}</p>{message && <p className="text-xs text-rose-600">{message}</p>}</div></div>{canUseModule(me, "orders") && <button onClick={() => setTab("orders")} className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-black text-white">Nova OS</button>}</div></header><section className="p-4 lg:p-8">{tab === "dashboard" && <DashboardView dashboard={dashboard} />}{tab === "clients" && <ClientsView data={clients} branchId={activeBranchId} reload={loadAll} loading={dataLoading} />}{tab === "employees" && <EmployeesView data={employees} branchId={activeBranchId} reload={loadAll} loading={dataLoading} />}{tab === "services" && <ServicesView data={services} branchId={activeBranchId} reload={loadAll} loading={dataLoading} />}{tab === "orders" && <OrdersView orders={orders} clients={clients} employees={employees} services={services} branchId={activeBranchId} reload={loadAll} onOrderSaved={applyOrderSaved} loading={dataLoading} />}{tab === "calendar" && <CalendarView branchId={activeBranchId} employees={employees} clients={clients} services={services} orders={orders} reload={loadAll} onOrderSaved={applyOrderSaved} />}{tab === "finance" && <FinanceView data={transactions} orders={orders} employees={employees} clients={clients} services={services} branchId={activeBranchId} reload={loadAll} onOrderSaved={applyOrderSaved} loading={dataLoading} />}{tab === "reports" && <ReportsView employees={employees} services={services} isGlobalAdmin={!!me?.isGlobalAdmin} />}{tab === "branches" && <BranchesView />}{tab === "users" && <UsersView />}</section></main>
  </div>;
}

// One card per status bucket, each with Quantidade + Valor total. Every
// number here comes straight from GET /api/dashboard (Postgres COUNT / _sum,
// already scoped to the active branch) - nothing is counted or summed on the
// client. Colors are deliberately distinct so "Total", "Agendadas",
// "Confirmadas" and "Realizadas" never read as the same figure.
function OrderBucketCard({ label, bucket, tone }: { label: string; bucket: OrderStatusBucket; tone: "slate" | "blue" | "emerald" | "violet" }) {
  const styles = {
...
const EMPTY_BUCKET: OrderStatusBucket = { count: 0, total: 0 };

// Totais de OS por status da filial ativa. "Total de OS" = todas as OS nao
// excluidas com status != cancelado (pendente + confirmado + em_andamento +
// finalizado), sempre >= a soma das tres colunas mostradas.
function OrderTotalsSection({ dashboard }: { dashboard: Dashboard }) {
  return <div className="rounded-2xl border bg-white p-5 shadow-sm">
    <h3 className="font-black">Totais de OS da filial</h3>
    <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <OrderBucketCard label="Total de OS (exceto canceladas)" bucket={dashboard.totalOrders ?? EMPTY_BUCKET} tone="slate" />
      <OrderBucketCard label="OS Agendadas" bucket={dashboard.scheduledOrders ?? EMPTY_BUCKET} tone="blue" />
      <OrderBucketCard label="OS Confirmadas" bucket={dashboard.confirmedOrders ?? EMPTY_BUCKET} tone="emerald" />
      <OrderBucketCard label="OS Realizadas" bucket={dashboard.finalizedOrders ?? EMPTY_BUCKET} tone="violet" />
    </div>
  </div>;
}
...
function ClientsView({ data, branchId, reload, loading }: { data: Client[]; branchId: string | null; reload: () => Promise<void>; loading: boolean }) {
  ...
  async function submit(e: FormEvent) {
    ...
    // Same required set the server enforces (clientSchema) - the text inputs
    // already have `required`, this only adds the one the <select> can't: UF.
    if (!form.addressState) { setError("Selecione o estado (UF) do endereco."); return; }
    ...
    if (data.some((c) => c.id !== editing?.id && isDuplicateClient({ name: form.name, document: form.document }, c))) {
      setError(DUPLICATE_CLIENT_MESSAGE); return;
    }
    try { const payload = editing ? form : { ...form, branchId }; await api(editing ? `/api/clients/${editing.id}` : "/api/clients", { method: editing ? "PUT" : "POST", body: JSON.stringify(payload) }); setOpen(false); setEditing(null); await reload(); } catch (err) { setError(err instanceof Error ? err.message : "Erro ao salvar cliente."); }
  }
  ...
  return <CrudShell title="Clientes" onNew={() => { setEditing(null); setError(""); setForm(BLANK_CLIENT_FORM); setOpen(true); }}>{data.map((c) => <div key={c.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap justify-between gap-2"><h3 className="font-black">{c.name}</h3><ActionButtons onEdit={() => { setEditing(c); setError(""); setForm(clientToForm(c)); setOpen(true); }} onDelete={() => remove(c.id)} /></div><p className="mt-2 text-sm text-slate-500">{c.phone} | {c.email}</p><p className="text-sm text-slate-500">{c.document}</p>{c.address && <p className="mt-2 text-sm text-slate-500">{c.address}</p>}<p className="mt-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-500">{c.notes || "Sem observacoes"}</p></div>)}{open && <Modal title={editing ? "Editar cliente" : "Novo cliente"} onClose={() => setOpen(false)}><form onSubmit={submit} className="grid gap-3">{error && <p className="rounded-xl bg-rose-50 p-3 text-sm font-bold text-rose-600">{error}</p>}<Input label="Nome" value={form.name} set={(v) => setForm({ ...form, name: v })} required /><Input label="Email" value={form.email} set={(v) => setForm({ ...form, email: v })} /><Input label="Telefone" value={form.phone} set={(v) => setForm({ ...form, phone: v })} /><Input label="CPF/CNPJ" value={form.document} set={(v) => setForm({ ...form, document: v })} /><div className="rounded-2xl bg-slate-50 p-4"><h4 className="font-black">Endereco</h4>{editing && !hasStructuredClientAddress(form) && editing.address && <p className="mt-1 text-xs text-slate-500">Endereco anterior (nao estruturado): <b>{editing.address}</b> - preencha os campos abaixo para atualizar.</p>}<div className="mt-3 grid gap-3 md:grid-cols-3"><CepField value={form.addressZip} onChange={(v) => setForm({ ...form, addressZip: v })} onFound={(address) => setForm({ ...form, addressZip: address.cep, addressStreet: address.street, addressNeighborhood: address.neighborhood, addressCity: address.city, addressState: address.state })} /><div className="md:col-span-2"><Input label="Rua / Logradouro" value={form.addressStreet} set={(v) => setForm({ ...form, addressStreet: v })} required /></div></div><div className="mt-3 grid gap-3 md:grid-cols-3"><Input label="Numero" value={form.addressNumber} set={(v) => setForm({ ...form, addressNumber: v })} required /><Input label="Bairro" value={form.addressNeighborhood} set={(v) => setForm({ ...form, addressNeighborhood: v })} required /><label className="grid gap-1 text-xs font-black uppercase text-slate-500">Estado (UF)<Select value={form.addressState} set={(v) => setForm({ ...form, addressState: v })} options={brazilStateOptions} /></label></div><div className="mt-3 grid gap-3 md:grid-cols-2"><Input label="Cidade" value={form.addressCity} set={(v) => setForm({ ...form, addressCity: v })} required /><Input label="Ponto de referencia (opcional)" value={form.addressReference} set={(v) => setForm({ ...form, addressReference: v })} /></div></div><Text label="Observacoes" value={form.notes} set={(v) => setForm({ ...form, notes: v })} /><Save /></form></Modal>}</CrudShell>;
}
...
function OrderFormModal({ order, orders, clients, employees, services, branchId, onClose, reload, onSaved }: { order: Order | null; orders: Order[]; clients: Client[]; employees: Employee[]; services: Service[]; branchId: string | null; onClose: () => void; reload: () => Promise<void>; onSaved: (order: Order) => Promise<void> }) {
  const blank = { clientId: clients[0]?.id || "", eventDate: dateOnly(), startTime: "18:00", endTime: "23:59", addressZip: "", addressStreet: "", addressNumber: "", addressNeighborhood: "", addressCity: "", addressState: "", addressReference: "", status: "pendente", paymentMethod: "", notes: "", signatureName: "", employeeIds: [] as string[], items: [] as Array<{ serviceId: string; quantity: number; unitPrice: number }> };
  // Orders created before the structured address fields existed have all of
  // them null - left blank here rather than parsed out of the legacy
  // `location` text, which would risk inventing a street/number/neighborhood
  // split that was never actually confirmed.
  function fromOrder(o: Order) { return { clientId: o.clientId, eventDate: dateOnly(o.eventDate), startTime: o.startTime, endTime: o.endTime, addressZip: o.addressZip || "", addressStreet: o.addressStreet || "", addressNumber: o.addressNumber || "", addressNeighborhood: o.addressNeighborhood || "", addressCity: o.addressCity || "", addressState: o.addressState || "", addressReference: o.addressReference || "", status: o.status, paymentMethod: o.paymentMethod || "", notes: o.notes || "", signatureName: o.signatureName || "", employeeIds: o.employees.map((x) => x.employee.id), items: o.items.map((i) => ({ serviceId: i.serviceId, quantity: i.quantity, unitPrice: Number(i.unitPrice) })) }; }
...
function CalendarView({ branchId, employees, clients, services, orders, reload, onOrderSaved }: { branchId: string | null; employees: Employee[]; clients: Client[]; services: Service[]; orders: Order[]; reload: () => Promise<void>; onOrderSaved: (order: Order) => Promise<void> }) {
  ...
  return <div className="space-y-5">
    ...
    {editingOrder && <OrderFormModal order={editingOrder} orders={orders} clients={clients} employees={employees} services={services} branchId={branchId} onClose={() => setEditingOrder(null)} reload={reloadCalendar} onSaved={handleOrderSaved} />}
  </div>;
}
...
function FinanceView({ data, orders, employees, clients, services, branchId, reload, onOrderSaved, loading }: { data: Transaction[]; orders: Order[]; employees: Employee[]; clients: Client[]; services: Service[]; branchId: string | null; reload: () => Promise<void>; onOrderSaved: (order: Order) => Promise<void>; loading: boolean }) { const [open, setOpen] = useState(false); const [editing, setEditing] = useState<Transaction | null>(null); const [viewOrder, setViewOrder] = useState<Order | null>(null); const [form, setForm] = useState({ type: "receita", category: "Eventos", description: "", amount: 0, dueDate: dateOnly(), paidAt: null as string | null, status: "pago", orderId: "" }); const [error, setError] = useState("");
