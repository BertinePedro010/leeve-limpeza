...
import { statusLabels, money, dateOnly, dateOnlyLabel, api, ApiError, Badge, Modal, Stat, CrudShell, ActionButtons, Input, NumberInput, Text, Select, Save, paymentMethodLabel, paymentMethodOptions, EmployeeMultiSelect, CepField, composeCepAddress } from "@/components/ui";
...
function OrderFormModal({ order, orders, clients, employees, services, branchId, onClose, reload }: { order: Order | null; orders: Order[]; clients: Client[]; employees: Employee[]; services: Service[]; branchId: string | null; onClose: () => void; reload: () => Promise<void> }) {
  ...
  // CEP is a lookup-only convenience, never persisted with the OS - the
  // resulting address still goes into the existing `location` field, so no
  // new column or API payload shape is needed on save.
